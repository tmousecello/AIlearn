...
適合解決分類問題(classification)  
也可解決回歸問題  
目前學到的分類技術:KNN、SVM、決策樹、隨機森林)(還有一堆)  
回歸技術:SVR、線性回歸
分類、回歸問題都屬監督式學習  
...
### 隨機森林
從抽樣過後的特徵集(所有特徵的一個子集)，來進行分支  
原因:為了要有群體智慧，發揮集成學習的效果，要產生不同的決策樹  
從不同角度看事情來投票，進而提升準確率  
給定有N個樣本的資料集，每個樣本有F個特徵，訓練有M棵決策樹的模型  
重複抽樣樣本N次->產生抽樣資料集->進行決策樹訓練->隨機抽樣特徵F次(特徵裝袋)->重複M次(產生M個模型)->各個決策樹投票  
